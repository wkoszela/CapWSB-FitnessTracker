...
class TrainingApiIntegrationTest extends IntegrationTestBase {
    ...
    void shouldReturnAllTrainings_whenGettingAllTrainings() throws Exception {
        ...
        mockMvc.perform(get("/v1/trainings").contentType(MediaType.APPLICATION_JSON))
                ...
                .andExpect(jsonPath("$[10].user.id").value(user1.getId()))
                .andExpect(jsonPath("$[10].user.firstName").value(user1.getFirstName()))
                .andExpect(jsonPath("$[10].user.lastName").value(user1.getLastName()))
                .andExpect(jsonPath("$[10].user.email").value(user1.getEmail()))
                .andExpect(jsonPath("$[10].distance").value((training1.getDistance())))
                .andExpect(jsonPath("$[10].averageSpeed").value(training1.getAverageSpeed()))
    ...
    void shouldReturnAllFinishedTrainingsAfterTime_whenGettingAllFinishedTrainingsAfterTime() throws Exception {
        ...
        List<Training> allTrainingDebug = getAllTrainings();

        mockMvc.perform(get("/v1/trainings/completed/{afterTime}", "2024-05-18").contentType(MediaType.APPLICATION_JSON))
                .andDo(log())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray())
                .andExpect(jsonPath("$[0].user.id").value(user1.getId()))
