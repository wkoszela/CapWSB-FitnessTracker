...
class TrainingApiIntegrationTest extends IntegrationTestBase {
    ...
    void shouldReturnAllTrainings_whenGettingAllTrainings() throws Exception {
        ...
        mockMvc
            .perform(get("/v1/trainings").contentType(MediaType.APPLICATION_JSON))
            .andDo(log())
    ...
    void shouldReturnAllTrainingsForDedicatedUser_whenGettingAllTrainingsForDedicatedUser() throws Exception {
        ...
        sdf.setTimeZone(java.util.TimeZone.getTimeZone("UTC"));
        mockMvc
            .perform(get("/v1/trainings/{userId}", user1.getId()).contentType(MediaType.APPLICATION_JSON))
            .andDo(log())
            .andExpect(status().isOk())
            .andExpect(jsonPath("$").isArray())
            .andExpect(jsonPath("$[0].user.id").value(user1.getId()))
    ...
    void shouldReturnAllFinishedTrainingsAfterTime_whenGettingAllFinishedTrainingsAfterTime() throws Exception {
        ...
        User user1 = existingUser(generateClient());
        Training training1 = persistTraining(generateTrainingWithDetails(
            user1,
            "2024-05-19 19:00:00",
            "2024-05-19 20:30:00",
        ...
        Training training2 = persistTraining(generateTrainingWithDetails(
            user1,
            "2024-05-17 19:00:00",
            "2024-05-17 20:30:00",
            ActivityType.RUNNING,
        ...
        sdf.setTimeZone(java.util.TimeZone.getTimeZone("UTC"));
        mockMvc
            .perform(get("/v1/trainings/finished/{afterTime}", "2024-05-18").contentType(MediaType.APPLICATION_JSON))
            .andDo(log())
            .andExpect(status().isOk())
            .andExpect(jsonPath("$").isArray())
            .andExpect(jsonPath("$[0].user.id").value(user1.getId()))
    ...
    void getAllTrainingByActivityType_whenGettingAllTrainingByActivityType() throws Exception {
        ...
        mockMvc
            .perform(
                get("/v1/trainings/activityType")
                    .param("activityType", "TENNIS")
            ...
            .andExpect(jsonPath("$[0].user.lastName").value(user1.getLastName()))
            .andExpect(jsonPath("$[0].user.email").value(user1.getEmail()))
            .andExpect(jsonPath("$[0].activityType").value(training2
                .getActivityType()
                .toString()))
            ...
            .andExpect(jsonPath("$[1].activityType").value(training3
                .getActivityType()
                .toString()))
    ...
    void shouldUpdateTraining_whenUpdatingTraining() throws Exception {
        ...
        String requestBody = """
            ...
            """.formatted(user1.getId());
        mockMvc
            .perform(put("/v1/trainings/{trainingId}", training1.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(requestBody))
            .andDo(log())
            .andExpect(status().isOk())
